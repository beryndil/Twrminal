...
- [x] `frontend/src/lib/components/Conversation.svelte` — **389
  ...
  - `parseUriList` / `extractPaths` / `hasFiles` →
    `frontend/src/lib/utils/composer-dragdrop.ts` (89 lines) +
    `composer-dragdrop.test.ts` (15 cases).
...
- [x] **Attachment chip above user bubble.** **Shipped 2026-04-30
  (`9527ada`).** Persistent chips render below the sent user bubble
  carrying filename + size + a click-to-preview affordance.
