...
* **Modifier-equivalence on Mac.** `Ctrl` and `Cmd` are treated as equivalent on platforms where `Cmd` is the primary modifier — the documented `Ctrl+Shift+P` works for a Mac visitor without a separate table. The cheat sheet renders the Cmd glyph alongside Ctrl for every Ctrl chord (e.g. `[⌘/Ctrl][Shift][P]`), so a Mac user immediately sees that `Cmd` is a valid modifier without having to discover it by experiment.
