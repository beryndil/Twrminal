...
* **Keyboard.** Up / Down navigate; Enter activates; Right opens a submenu; Esc closes. Typing a single letter or digit jumps the highlight to the first action whose mnemonic matches (case-insensitive). Repeated presses of the same key cycle through all actions sharing that mnemonic. Each action's mnemonic is the first alphanumeric character of its label (overridable per action via the `mnemonic` field on `MenuActionDescriptor`); the matching character is rendered underlined in the menu.
